...
function fetchRandomPost() {
    if (randomPostWorking === false) {
        ...
        var randomRotate = randomPostTimes * 360;
        var randomPostTipsItem = randomPostTips[Math.floor(Math.random() * randomPostTips.length)];
        var randomPostLevel = "";
        if (randomPostTimes > 1e4) {
            randomPostLevel = "愿者上钩"
        } else if (randomPostTimes > 1e3) {
        ...
        var randomTime = randomNum(1e3, 3e3);
        if (randomPostTimes === 0) {
            randomTime = 0
        }
        document.getElementById("random-post-start").style.opacity = "0.2";
            ...
            fetch(fetchUrl).then(function (res) {
                ...
                if (document.querySelector("#random-post")) {
                    window.setTimeout(function () {
                        ...
                        document.getElementById("random-post-start").style.opacity = "1"
                    }, randomTime)
                }
            });
            randomPostWorking = false
...
function initRandomPost() {
    if (localStorage.randomPostTimes) {
        ...
        document.getElementById("random-post-start").style.transitionDuration = "0.3s";
        document.getElementById("random-post-start").style.transform = "rotate(" + randomPostTimes * 360 + "deg)"
    }
    fetchRandomPost()
}
