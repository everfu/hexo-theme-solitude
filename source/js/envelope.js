class EasyDanmaku {
    constructor(params) {
      ...
      this.pathname = params.page || null;
    ...
    batchSend(list, hasAvatar = false, normalClass = null) {
      ...
      let timer = setInterval(() => {
        if (location.pathname != this.pathname) clearInterval(timer);
        if (this.originIndex > list.length - 1) {
          ...
          if (hasAvatar) {
            this.send(`${list[this.originIndex].url ? '<a href="' + list[this.originIndex].url + '">' : ''}<img src=${list[this.originIndex].avatar}>
                          <p>${list[this.originIndex].content}</p>${list[this.originIndex].url ? '</a>' : ''}
  ...
  class Utils {
    ...
    static eventDelegation(parent, childName, EventName, callBackFn) {
      parent.addEventListener(EventName, (e) => {
        try {
          if (e.target.className.includes(childName)) {
            callBackFn(e.target)
          } else if (e.target.parentNode.className.includes(childName)) {
            callBackFn(e.target.parentNode)
          }
        } catch (error) { }
      })
    }
